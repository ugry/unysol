...
export default function DriverLeavePage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try { await api.post('/api/tenant/driver-leave', form); setShowModal(false); setForm({ user_id: 0, baslangic: '', bitis: '', turu: 'YILLIK_IZIN', aciklama: '' }); fetchData(); }
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white border border-enterprise-border rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5"><h3 className="text-lg font-semibold">İzin Ekle</h3><button onClick={() => setShowModal(false)}><X size={20} /></button></div>
