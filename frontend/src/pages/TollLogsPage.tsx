...
export default function TollLogsPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    try { if (editingId) await api.put(`/api/tenant/toll-logs/${editingId}`, { ...form, gecis_ucreti: parseFloat(form.gecis_ucreti) || 0 }); else await api.post('/api/tenant/toll-logs', { ...form, gecis_ucreti: parseFloat(form.gecis_ucreti) || 0 }); setShowModal(false); setEditingId(null); setForm({ truck_id: 0, gecis_tarihi: '', hgs_etiket_no: '', giris_gise: '', cikis_gise: '', gecis_ucreti: '' }); fetchData(); }
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      <div className="flex items-center justify-between"><div /><button onClick={() => setShowModal(true)} className="flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#FF5F03] hover:bg-[#E55600] text-white font-medium text-sm"><Plus size={18} /> HGS Ekle</button></div>
      <DataGrid columns={columns} data={data} loading={loading} title="HGS Geçiş Takibi" emptyIcon={<CreditCard size={48} className="text-gray-300" />} emptyText="Henüz HGS kaydı yok"
        onEdit={(row) => { setEditingId(row.id); setForm({ truck_id: row.truck_id, gecis_tarihi: row.gecis_tarihi?.substring(0,10) || '', hgs_etiket_no: row.hgs_etiket_no || '', giris_gise: row.giris_gise || '', cikis_gise: row.cikis_gise || '', gecis_ucreti: String(row.gecis_ucreti || '') }); setShowModal(true); }}
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm">
          <div className="w-full max-w-lg bg-white border border-enterprise-border rounded-2xl p-6">
            <div className="flex items-center justify-between mb-5"><h3 className="text-lg font-semibold">{editingId ? 'HGS Geçişi Düzenle' : 'HGS Geçişi Ekle'}</h3><button onClick={() => setShowModal(false)}><X size={20} /></button></div>
